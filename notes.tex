...
Vertical {\bf feature extraction} is possible using a trivial {\bf mask} (or {\bf filter} or {\bf kernel})$[+\vert -]$ which is applied on an $m\times n$ image and returns an $(m-1)\times n$ images with all columns containing the difference of the original columns. Usually, the absolute value of the difference is computed. \\
A {\bf gradient image} is the aggregation of vertical and horizontal filtering. \\
