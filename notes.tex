...
Supervised learning learns from training data If there are not sufficiently enough training data, the learning algorithm may {\bf underfit}. In contrast, it may {\bf overfit} for too many training data meaning it only matches the truing data well and provides distorted results for new data.

\subsection{Linear regression}
There are $m$ training examples having $n+1$ features. Linear regression computes a regression function for these training examples. The objective is to minimize the cost function:
...
{\bf Vectorization} allows to do the updates simultaneously by all updates through various matrix operations. Matrix multiplications can be executed very efficiently (Strassen algorithm).
...
The {\bf normal equation} is a closed-form solution for linear regression which requires no loop, no learning rate and no feature normalization.
