...
\section{k-means}
{\bf k-means} partitions $m$ observations into $K$ clusters as defined in Algorithm~\ref{ref:kmeans}.  The optimization objective is to minimize the cost function.

\begin{algorithm}
\caption{k-means}
\label{ref:kmeans}
\begin{algorithmic}
\State $\mu_1, \mu_2, ..., \mu_K \in \mathbb{R} \gets$ random initialization
\Repeat
\For{$i=1$ to $m$}
