...
Supervised learning learns from {\bf labeled training examples}. If there are not sufficiently enough tfeatures, the learning algorithm may {\bf underfit}. In contrast, it may {\bf overfit} for too many features meaning it only matches the training data well but fails to generalize to new examples.
...
If $x\approx l^{(i)}$, then $f_i\approx 1$. If $x$ far from $l^{(i)}$, then $f_i\approx 0$. To optimize the training, the regularization part is slightly changed:
...
The {\bf principal component analysis} (PCA) presented in Section~\ref{ref:pcasection} can also be used in supervised learning.


\chapter{Unsupervised learning}
Unsupervised learning tries to find hidden structure in {\bf unlabeled data} $\{x^{(1)}, x^{(2)}, ..., x^{(m)}\}$.
...
\label{ref:pca}
\begin{algorithmic}
\State $K \gets$ number of principal components
...
\State $z \gets U_{reduce}^Tx$ \Comment{Maps data $x \in \mathbb{R}^n$ to $z\in \mathbb{R}^K$}
\end{algorithmic}
\end{algorithm}
...
\end{align*}
This measure is more useful than the actual amount of dimensions that were reduced. Then, the smalls $K$ to satisfy this equation is chosen. Computing $K$ this way is inefficient as PCA might need to be performed  a lot times to choose the smallest $K$. Alternatively, the retained variance can be computed for a given $K$:
\begin{align*}
