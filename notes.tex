...
The {\bf shortest path algorithm} finds the shortest path from the {\bf start state} to the {\bf goal state}.
...
\item {\em successors}: function which returns the successors of a start
\item {\em is\_goal}: function which returns true of a state is the goal
\end{itemize}
The {\bf frontier} contains all successors of visited states. The frontier is sorted by path length in ascending order.
Visited states will not be visited again.
