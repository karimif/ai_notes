...
\\
Figure~\ref{fig:samplehiddenmarkov} contains a hidden Markov model in which the measurements are happy (H) or grumpy (G) depending on the weather rainy (R) or sunny (S). Given measurements, the state probabilities can be calculated given Bayes rule, e.g.: $P(R_1\vert H_1)=\frac{P(H_1\vert R_1)P(R_1)}{P(H_1)}$.
...
HMMs are used for {\bf localization} and {\bf tracking} which is covered by Chapter~\ref{chapter:localizationandtracking}.
