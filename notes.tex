...
Markov localization is computationally cheap but does not scale for higher dimensions. \\
...
\end{align*}
The motion itself is uncertain and can be described by a Gaussian $\mu_2, \sigma_2^2$. Algorithm~\ref{ref:1dkalmanfilterpython} implements a sample 1d Kalman filter. The velocity is inferred from measuring different positions. Higher-dimensional Kalman filters also allow to infer properties to predict the future, e.g. the velocity in 2d measurements. In that case, Kalman filters allow to predict the velocity of other vehicles which helps a robot to avoid accidents.
...
Multi-dimensional Kalman filters can efficiently be computed through matrix operations. The following elements are needed: {\bf estimate} $x$, {\bf uncertainty covariance} $P$, {\bf state transition matrix} $F$, {\bf motion vector} $u$, {\bf measurement} $z$, {\bf measurement function} $H$ and {\bf measurement noise} $R$. \\
...
Algorithm~\ref{ref:2dkalmanfilterpython} implements a sample 2d Kalman filter which infers the velocity of the object.
