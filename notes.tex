...
\\ \\
"Small" {\bf neural networks} which have fewer parameters are prone to underfitting but computationally chapter. "Large" neural networks have more parameters and are more prone to overfitting (can be addressed by regularization) but computationally more expensive.
\\ \\
...
A {\bf support vector machine} (SVM) is a large margin linear classifier. In order to get a large margin, the motivation for the cost function definition is:
\begin{definition}[SVM hypothesis]~\\
$h_{\theta}(x) =
\left\{
\begin{array}{lll}
1  & \mbox{if } \theta^Tx \ge 1$ (not just $\ge $0)$ \\
0  & \mbox{if } \theta^Tx \le -1$ (not just $\le $0)$
\end{array}
\right.$ \\
\end{definition}
...
\begin{definition}[Optimized SVM cost function]~\\
$J(\theta)=C\sum_{i=1}^m[y^{(i)}cost_1(\theta^Tx^{(i)})+(1-y^{(i)})cost_0(\theta^Tx^{(i)})]+\frac{1}{2}\sum_{j=1}^{\bf m}\theta_j^2$
\end{definition}
...
Unsupervised learning tries to find hidden structure in {\bf unlabeled data}.
