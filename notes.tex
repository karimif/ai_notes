...
{\bf Anomaly detection} allows to find data that does not conform to an expected pattern. The cluster-based Algorithm~\ref{ref:anomalydetection} assumes Gaussian distribution selected features that take unusually large or small values in the event of an anomaly:
\begin{align*}
...
\State $\mu_j \gets \frac{1}{m}\sum_{i=1}^{m}x_j^{(i)}$ \Comment{Fit parameters $\mu_1,...,\mu_n,\sigma_1,...,\sigma_n$}
\State $\sigma_j^2 \gets \frac{1}{m}\sum_{i=1}^{m}(x_j^{(i)}-\mu_j)^2$
\State For new example $x$: $p(x) \gets \prod_{j=1}^np(x_j,\mu_j,\sigma_j^2)$ \Comment{Assumes independence between components}
...
\subsection{Anomaly detection using multivariate Gaussian distribution}
