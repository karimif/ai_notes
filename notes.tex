...
{\bf Markiv localization} is a grid-based localization which uses histogram to represent the belief distribution. Localization starts in an initial {\bf belief state} and consists of two steps: {\bf sense} and {\bf move}. Sense gains information on the current localization. In contrast, move loses information as movements are inaccurate. \\
The sense step is: \\
To come
...
\end{align*}
It maps rom a belief state $P(x_{j,t-1})$ and action $a_t$ to a new predicted belief state $P(x'_{i,t})$ by summing over all possible ways (i.e. from all states x$_{j,t-1}$) in which the robot may have reached $x'_{i,t}$.
