...
Supervised learning learns from training examples. If there are not sufficiently enough training examples, the learning algorithm may {\bf underfit}. In contrast, it may {\bf overfit} for too many training examples meaning it only matches the training data well and provides distorted results for new data.

\subsection{Linear regression}
{\bf Linear regression} computes a linear regression function for $m$ {\bf training examples} having $n+1$ {\bf features}. The objective is to minimize the cost function:
...
The parameters of the model to be learned are the $\theta_j$ values. One way to do this is to use the batch {\bf gradient descent} algorithm. Through is step, the parameters $\theta_j$ come closer to the optimal values that will achieve the lowest cost $J(\theta)$. If the {\bf learning rate} $\alpha$ is too large, gradient descent might not converge and overshoot the minimum after a certain amount of iterations. Gradient descent might generally not find the global but the local minimum.
...
{\bf Vectorization} allows to do the updates simultaneously by all updates through various matrix operations. Matrix multiplications can be executed very efficiently (Strassen algorithm). Matrix $X$ contains $m$ training data rows and $n+1$ features where row $x_0$ contains entirely 1s. The cost function and gradient descent then look:
...
{\bf Feature normalization} allows to perform gradient descent quickly by scaling each feature, e.g.:
\begin{definition}[Feature normalization]
$v' = \frac{v - \mu}{\sigma ^2}$
\end{definition}

The {\bf normal equation} is a closed-form solution for linear regression which requires no loop, no learning rate and no feature normalization. Is it usually recommended to calculate the pseudo inverse as the inverse of $X$ might not exist.
...
Logistic regression allows linear classification. The hypothesis is defined as:
\begin{definition}[Hypothesis]
$h_\theta(x) = g(\theta^{T}x)$
\end{definition}

$g$ is the {\bf sigmoid function} which is defined as:
