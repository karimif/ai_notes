...
\\
{\bf Greyscale images} are used in most computer science algorithms as they contain the relations between objects and are easier to process than RGB images. Values usually range from 0 to 255. \\ Vertical {\bf feature extraction} is possible using a trivial {\bf mask} $[+\vert -]$ which is applied on an $m\times n$ image and returns an $(m-1)\times n$ images with all columns containing the difference of the original columns. Usually, the absolute value of the difference is computed.
