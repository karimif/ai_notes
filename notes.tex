...
The {\bf shortest path algorithm} finds the shortest path from the {\bf start state} to the {\bf goal state} in terms of path elements which all have the same value.
Each state might have a number of {\bf successors}. Algorithm~\ref{python:shortestpath} implements the shortest path algorithm.
\\
...
\item {\em is\_goal}: function which returns true of a state is the goal
\end{itemize}
The {\bf frontier} contains all successors of previously visited states. The frontier is sorted by path length in ascending order. Visited states will not be visited again.
The algorithm picks the first element of the frontier and {\bf expands}, meaning to go through its successors.
A successor might be the goal state. If so, the algorithm returns the successful path.
...
\section{Generalizing search to solve problems}
To come.
...
\section{Example: applying logic to simplify a problem}
To come.
