...
The optimal policy is defined in Figure~\ref{ref:sampleworldoptimalpolicy}. The action of $(3, 1)$ is not obvious. If it was north, there would be a 0.1 chance to go to -1 from $(3, 2)$. Therefore, it is left to route away from this potential action.
...
{\bf Value iteration} is an efficient algorithm for solving finite-state MDPs as defined in Algorithm~\ref{ref:valueiterationalgorithm}.
...
\Repeat
\State $\forall s\in S:$ update $V(s) \gets R(s) + \max\limits_{a\in A} \gamma \sum_{s'} P_{sa}(s')V^{*}(s')$
\Until{conversion}
\end{algorithmic}
\end{algorithm}
